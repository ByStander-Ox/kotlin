...
class ArrayAccessAssignmentReceiver(
    ...
    class CompoundAssignmentInfo {
        val indexVariables = LinkedHashSet<IrVariable>()
        lateinit var lastIndexVariable: IrVariable
        var arrayGetCall: IrExpression? = null
        var arraySetCall: IrExpression? = null
    }
    ...
    override fun assign(withLValue: (LValue) -> IrExpression): IrExpression {
        ...
        if (indexedGetDescriptor?.isDynamic() != false && indexedSetDescriptor?.isDynamic() != false) {
            return withLValue(
                createLValue(kotlinType, OnceExpressionValue(irArray), null) { _, irIndex ->
        ...
        irBlock.inlineStatement(
            withLValue(
                createLValue(kotlinType, irArrayValue, compoundAssignmentInfo) { i, irIndex ->
                    ...
                    compoundAssignmentInfo.lastIndexVariable = irIndexVar
